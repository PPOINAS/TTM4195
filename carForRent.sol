...
contract carForRent is ERC721, Ownable {
     ...
     * @return carID The unique ID of the car (the NFT)
     */
    function createCar(
        string memory model,
        string memory color,
     ...
     */
    function calculateMonthlyPaymentAmount(uint256 carId) public view returns (uint256) {
        Lease storage currentLease = _leases[carId];
        require(currentLease.lessee != address(0), "No lease found for this car");
     ...
     * @param carId The ID of the leased car
     * @return state The state of the payment
     */
        ...
        external onlyOwner returns (PaymentState state) 
    {
        ...
        if ((block.timestamp > currentLease.nextPaymentDueDate + 60 days) || (currentLease.consecutiveMissedPayments >= MAX_MISSED_PAYMENTS_ALLOWED)) {
     ...
     * @param newDriverExperience The updated years of possession of a driving license
     */
     ...
     * @param newDriverExperience The years of possession of a driving license [years]
